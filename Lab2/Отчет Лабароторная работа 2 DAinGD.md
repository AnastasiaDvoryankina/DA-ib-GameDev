...
## Задание 1. # АНАЛИЗ ДАННЫХ И ИСКУССТВЕННЫЙ ИНТЕЛЛЕКТ [in GameDev]
Отчет по лабораторной работе #2 выполнил(а):
- Гаев Андрей Дмитриевич
- НМТ-213511 (АТ-04)
Отметка о выполнении заданий (заполняется студентом):
...
Обнаружила ошибку, которая не воспроизводила последнюю строчку в столбце. Элемент i != dataSet.Count говорит о том, что мы воспроизводим звук, если наш счетчик не больше колва строк. Однако в python списки начинаются с 1го элемента, а в C# с нулевого, поэтому последняя строчка в интерпритации на C# не отоброжалось. Я заметила это и исправила ситуацию путем добавления к крайнему положению счетчика единицу.
...
Ну и по скольку метод Update работает сразу, как только готов новый кадр, то мы получаем неограниченное число вызовов информации из гугл таблицы, надо бы как-то его ограничить. Например добавить, что ячейка в принципе имеет содержимое, т.е. [Содержимое] != null или [Статус подключения к таблице] == true, что работает с базами данных, но мои попытки воспроизвести этот метод потерпели крах.
...
## Выводы
Мной была создана система генерации - записи - считывания информации с помощью небольшого скрипта на пайтоне, запись сгенерированных данных в гугл-таблицу и считывание их в юнити. Эта система может быть использованна как функционал отладки при работе с данными в играх, при анализе фондовых рынков на основании выводов обученной модели и других манипуляциях с данными. Важным фактом является возможность взаимодействия машинных систем и баз данных представленных в нашей работе в виде гугл таблицы. Благодарю за внимание.
